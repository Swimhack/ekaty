...
async function debugRestaurantsPage() {
    ...
    try {
        ...
        // Check for specific containers that might hold restaurant data
                ...
                if (elements.length > 0) {
                    results.push({
                        ...
                        sample: elements[0] ? {
                            ...
                            textPreview: elements[0].textContent.trim().substring(0, 100) + '...'
                        } : null
                    });
                }
            });
        ...
        if (containers.length > 0) {
            containers.forEach(container => {
                console.log(`   ${container.selector}: ${container.count} elements found`);
                if (container.sample) {
                    console.log(`      Sample: <${container.sample.tagName}> class="${container.sample.className}"`);
                    console.log(`      Text: "${container.sample.textPreview}"`);
                }
                console.log('');
            });
        } else {
            console.log('   ❌ No common container patterns found!\n');
        }
